...
frugalia-mcp is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i frugalia-mcp:latest
   ```
...
kubectl get mcpserver frugalia-mcp
```